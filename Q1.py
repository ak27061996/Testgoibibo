...
Validaton engine for AI/ML based recommendation engine
A recommender system typically produces an ordered list of recommendations for each user in the test set. 
I will start by building a system that will hit the search api to get the recommended items and then evaluate the search results based on the mentioned below validation parameters.
Validators parameters I will use to evaluate the performance of the recommendation engine-
1.MAP@K gives insight into how relevant the list of recommended items are.
Hence Mean Average Precision at K (MAP@K) is typically the metric of choice for evaluating the performance of a recommender systems.I will use the same for my recommendation engine validator.
Relevant recommendations are defined as recommendations of items that the user has rated positively in the test data.
2.Coverage is the percent of items in the training data the model is able to recommend on a test set.
3.Personalization is a great way to assess if a model recommends many of the same items to different users. It is the dissimilarity (1- cosine similarity) between user’s lists of recommendations.
4.Intra-list similarity is the average cosine similarity of all items in a list of recommendations. This calculation uses features of the recommended items to calculate the similarity
